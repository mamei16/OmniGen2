...
class OmniGen2Pipeline(DiffusionPipeline):
    ...
    def quantize_transformer(self, bits: int, empty_init=False, device=None, **kwargs):
        if bits == 0:
            return

        from ..quantization import quantize_2d_transformer

        self.quantized = True

        self.config.quantization_bit = bits

        self.transformer = quantize_2d_transformer(self.transformer, bits, empty_init=empty_init, device=device,
                                            **kwargs)
        return self.transformer

    def quantize_mllm(self, bits: int, empty_init=False, device=None, **kwargs):
        if bits == 0:
            return
        ...
        self.mllm.model = quantize(self.mllm.model, bits, empty_init=empty_init, device=device,
                                            **kwargs)
        return self.mllm
    ...
    def encode_vae(self, img: torch.FloatTensor) -> torch.FloatTensor:
        ...
        #self.vae.to(self.transformer.dtype)
        return z0
    ...
    def processing(
        ...
        if not latents.dtype == torch.float32:
